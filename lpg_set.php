...
function update_counter_inc()
{
   $cntr = 0;
   $result = mysql_query("SELECT value FROM config WHERE id = '3'");
   if (!empty($result)) 
   {
       if (mysql_num_rows($result) > 0)
       {
           $it = mysql_fetch_array($result);
           $cntr = intval($it['value']) + 1;
       }
       mysql_free_result($result);
   }
   $result = mysql_query("UPDATE config SET value = '$cntr' WHERE id = '3'"); 
}
